...
def compile_gesture(gesture_spec: dict) -> str:
    ...
    if gesture_type == "Pan":
        ...
        lines.append(f"  .activeOffset{self._capitalize(direction)}({threshold})")
        lines.append(f"  .onEnd((event) => {{")
        lines.append(f"    if (event.velocity{self._capitalize(direction)} > 500) {{")
        ...
        lines.append(f"    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.{self._map_haptic(haptic)});")
        lines.append(f"    runOnJS(handleGesture)();")
        lines.append(f"  }});")
